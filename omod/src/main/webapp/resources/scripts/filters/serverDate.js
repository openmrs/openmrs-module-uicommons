angular.module('uicommons.filters').
    ...
    .filter('serverDateLocalized', ['locale', function(locale) {
        return function(isoString, options) {
            ...
            // Check if it's just a date string (YYYY-MM-DD format) without time component
            // if so, add time component of 00:00:00.000, see: https://openmrs.atlassian.net/browse/UICM-111
            if (/^\d{4}-\d{2}-\d{2}$/.test(isoString)) {
                isoString = isoString + 'T00:00:00.000';
            }
            // trim any time zone suffixes
            if (isoString.length > 23) {
                isoString = isoString.substring(0, 23);
            }
