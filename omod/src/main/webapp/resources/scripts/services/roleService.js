angular.module('roleService', ['ngResource'])
    ...
    .factory('RoleService', function(Role) {
        ...
        return {
            ...
            retireRole: function(role) {
                var uuid = typeof role === "string" ? role : role.uuid;
                return new Role({ uuid: uuid, retired: false, reason: role.reason  }).$save();
            },

            unretireRole: function(role) {
                var uuid = typeof role === "string" ? role : role.uuid;
                return new Role({ uuid: uuid, retired: false }).$save();
            }
        }
    });